...
// WaitModeForAckContract maps the externally documented offset boundary to
// the exact Engine.Execute wait mode. Callers must reject any other value.
func WaitModeForAckContract(contract AckContract) (effectusruntime.WaitMode, error) {
	switch contract {
	case AckAfterDurableAcceptance:
		return effectusruntime.WaitAccepted, nil
	case AckAfterCompletedProcessing:
		return effectusruntime.WaitTerminal, nil
	default:
		return "", fmt.Errorf("unsupported acknowledgement contract %q", contract)
	}
}
