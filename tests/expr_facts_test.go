...
func TestExprFactsEvaluation(t *testing.T) {
	...
	// Build a registry-backed provider
	facts := pathutil.NewRegistryFactProviderFromMap(map[string]interface{}{
		"customer": customerData,
		"order":    orderData,
	})
...
func TestJSONAdapter(t *testing.T) {
  ...
  "orders": [
	...
	err := json.Unmarshal(jsonData, &customerData)
	assert.NoError(t, err, "Should parse JSON data without error")

	// Build a provider with a namespace
	facts := pathutil.NewRegistryFactProviderFromMap(map[string]interface{}{
		"customer": customerData,
	})
	...
	env["customer"], _ = facts.Get("customer")
