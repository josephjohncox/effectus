...
func (p *PostgresStorage) StoreRuleset(ctx context.Context, ruleset *StoredRuleset) error {
	...
	if !ruleset.CompiledAt.IsZero() {
		compiledAt = &ruleset.CompiledAt
	}

	var gitCommit, gitBranch, gitTag, gitAuthor, pullRequest *string
	if ruleset.GitCommit != "" {
		gitCommit = &ruleset.GitCommit
	}
	if ruleset.GitBranch != "" {
		gitBranch = &ruleset.GitBranch
	}
	if ruleset.GitTag != "" {
		gitTag = &ruleset.GitTag
	}
	if ruleset.GitAuthor != "" {
		gitAuthor = &ruleset.GitAuthor
	}
	if ruleset.PullRequest != "" {
		pullRequest = &ruleset.PullRequest
	}
	...
	if ruleset.CreatedBy != "" {
		createdBy = &ruleset.CreatedBy
	}
	if ruleset.UpdatedBy != "" {
		updatedBy = &ruleset.UpdatedBy
	}

	var compilerVersion, schemaVersion, validationHash *string
	if ruleset.CompilerVersion != "" {
		compilerVersion = &ruleset.CompilerVersion
	}
	if ruleset.SchemaVersion != "" {
		schemaVersion = &ruleset.SchemaVersion
	}
	if ruleset.ValidationHash != "" {
		validationHash = &ruleset.ValidationHash
	}

	// Use upsert to handle conflicts
	dbRuleset, err := p.queries.UpsertRuleset(ctx, db.UpsertRulesetParams{
		Name:            ruleset.Name,
		Version:         ruleset.Version,
		Environment:     ruleset.Environment,
		Status:          db.RulesetStatus(ruleset.Status),
		RulesetData:     rulesetData,
		RuleCount:       int32(len(ruleset.Ruleset.Rules)),
...
func (p *PostgresStorage) GetRuleset(ctx context.Context, name, version string) (*StoredRuleset, error) {
	...
	for _, env := range environments {
		dbRuleset, err := p.queries.GetRuleset(ctx, db.GetRulesetParams{
			Name:        name,
			Version:     version,
			Environment: env,
		})
...
func (p *PostgresStorage) ListRulesets(ctx context.Context, filters *RulesetFilters) ([]*RulesetMetadata, error) {
	...
	var owner, team, createdBy, gitCommit *string
	if filters.Owner != "" {
		owner = &filters.Owner
	}
	if filters.Team != "" {
	...
	// Execute query
	dbRulesets, err := p.queries.ListRulesets(ctx, db.ListRulesetsParams{
		Column1:   names,
		Column2:   environments,
		Column3:   status,
		Owner:     owner,
		Team:      team,
		Tags:      filters.Tags,
...
func (p *PostgresStorage) RecordActivity(ctx context.Context, entry *AuditEntry) error {
	...
	var version, environment, userID, userEmail, userAgent, sessionID *string
	...
	if entry.DurationMs > 0 {
		duration := int32(entry.DurationMs)
		durationMs = &duration
	}

	_, err = p.queries.CreateAuditEntry(ctx, db.CreateAuditEntryParams{
		Action:       entry.Action,
...
func (p *PostgresStorage) GetAuditLog(ctx context.Context, filters *AuditFilters) ([]*AuditEntry, error) {
	...
	// Execute query
	dbEntries, err := p.queries.GetAuditLogs(ctx, db.GetAuditLogsParams{
		Column1:     actions,
		Column2:     resources,
		Column3:     userIDs,
		Timestamp:   startTime,
...
func (p *PostgresStorage) Cleanup(ctx context.Context, olderThan time.Time) error {
	...
	if err := p.queries.CleanupOldAuditLogs(ctx, pgx.NullTime{Time: olderThan, Valid: true}); err != nil {
...
func getInt32Value(i *int32) int {
	if i == nil {
		return 0
	}
	return int(*i)
