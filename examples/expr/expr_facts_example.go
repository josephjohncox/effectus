...
func main() {
	// Example 1: Using registry-backed facts
	fmt.Println("=== Example 1: Basic Registry Facts ===")
	basicExample()

	// Example 2: Inspecting types via the registry
	fmt.Println("\n=== Example 2: Registry Type Information ===")
	typedExample()
...
func basicExample() {
	...
	// Create registry-backed facts provider
	facts := pathutil.NewRegistryFactProviderFromMap(data)
...
func typedExample() {
	...
	for _, path := range paths {
		...
		typeInfo, ok := facts.GetRegistry().GetType(path)
		typeName := "unknown"
		if ok {
	...
	err := facts.GetRegistry().TypeCheckExpression(expr)
	if err != nil {
	...
	result, err := facts.EvaluateExpr("metrics.count * 2")
	if err != nil {
		fmt.Printf("Evaluation error: %v\n", err)
	} else {
		resultType := pathutil.NormalizeTypeName(fmt.Sprintf("%T", result))
...
func jsonExample() {
	...
	// Load into registry-backed provider
	...
	for i := 0; i < 2; i++ {
		...
		name, _ := facts.Get(productPath + ".name")
		price, _ := facts.Get(productPath + ".price")
		inStock, _ := facts.Get(productPath + ".in_stock")
		firstTag, _ := facts.Get(productPath + ".tags[0]")

		fmt.Printf("  - %s: $%.2f (In stock: %t, Category: %s)\n", name, price, inStock, firstTag)

		// Get specs
		cpu, _ := facts.Get(productPath + ".specs.cpu")
		ram, _ := facts.Get(productPath + ".specs.ram")
		storage, _ := facts.Get(productPath + ".specs.storage")
		fmt.Printf("    Specs: CPU: %s, RAM: %d GB, Storage: %d GB\n", cpu, ram, storage)
	}

	// Store information
	storeName, _ := facts.Get("store.name")
	storeLocation, _ := facts.Get("store.location")
...
func structExample() {
	...
	location, _ := facts.Get("location")
	updated, _ := facts.Get("updated_at")
	...
	for i := 0; i < int(count.(int)); i++ {
		name, _ := facts.Get(fmt.Sprintf("products[%d].name", i))
		price, _ := facts.Get(fmt.Sprintf("products[%d].price", i))
		inStock, _ := facts.Get(fmt.Sprintf("products[%d].in_stock", i))
		category, _ := facts.Get(fmt.Sprintf("products[%d].category", i))
...
func namespacedExample() {
	...
	userName, _ := facts.Get("user.name")
	userRoles, _ := facts.Get("user.roles")
	cacheEnabled, _ := facts.Get("config.cache_enabled")
	visits, _ := facts.Get("stats.visits")
	topPage, _ := facts.Get("stats.top_pages[0]")
